...
<div class="space-y-12">
    <div class="flex items-center gap-6">
        ...
        <div>
            <h1 class="text-3xl font-heading font-black text-white tracking-tight">New <span class="text-primary italic">Testimonial.</span></h1>
            <p class="text-slate-500 font-medium uppercase text-[10px] tracking-[0.3em]">Add client review</p>
        </div>
    </div>

    <form action="{{ route('admin.testimonials.store') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-12" x-data="{ photoPreview: null }">
        @csrf
        <div class="lg:col-span-2 space-y-8">
            <div class="bg-slate-900/50 p-8 sm:p-12 rounded-[3rem] border border-white/5 backdrop-blur-xl space-y-8">
                <div>
                    ...
                    <input type="text" name="name" required value="{{ old('name') }}" placeholder="e.g. Bpk. Joko Widodo"
                ...
                <div>
                    ...
                    <textarea name="content" rows="6" required placeholder="Layanan sangat profesional dan cepat..."
                              class="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-white focus:outline-none focus:border-primary/50 transition-all font-medium">{{ old('content') }}</textarea>
                </div>
            </div>
        </div>
            ...
            <div class="bg-slate-900/50 p-10 rounded-[3rem] border border-white/5 space-y-10">
                <div>
                    ...
                    <select name="rating" required class="w-full bg-white/5 border border-white/10 rounded-xl px-6 py-4 text-white focus:outline-none focus:border-primary/50 transition-all font-bold">
                        <option value="5" class="bg-slate-900">★★★★★ (5 Stars)</option>
                        <option value="4" class="bg-slate-900">★★★★☆ (4 Stars)</option>
                        <option value="3" class="bg-slate-900">★★★☆☆ (3 Stars)</option>
                        <option value="2" class="bg-slate-900">★★☆☆☆ (2 Stars)</option>
                        <option value="1" class="bg-slate-900">★☆☆☆☆ (1 Star)</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <div x-show="photoPreview" class="mb-6 rounded-full overflow-hidden border border-white/10 relative w-24 h-24 mx-auto" style="display: none;">
